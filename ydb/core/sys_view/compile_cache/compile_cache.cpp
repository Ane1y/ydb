...
private:
    ...
    bool CanAccessEntry(const TCompileCacheQuery& entry) const {
        // A shared node may cache queries from several databases. Restrict every
        // reader, including admins and internal requests, to the query database.
        // Entries without a database cannot be safely attributed to this view.
        if (!entry.HasDatabase() || entry.GetDatabase().empty() || entry.GetDatabase() != DatabaseName) {
            return false;
        }
